...
- `/admin` — operator overrides; password-gated (login form). Password comes from `NEXT_PUBLIC_ADMIN_PASSWORD` (falls back to `NEXT_PUBLIC_ADMIN_KEY`) in `.env.local`. Note: `NEXT_PUBLIC_*` is inlined into the client bundle, so this gate is convenience, not real security.
